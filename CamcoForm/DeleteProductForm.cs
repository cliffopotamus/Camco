...
namespace CamcoForm
{
    public partial class DeleteProductForm : Form
    {
        ...
        {
            InventoryBox ib = (InventoryBox)comboInventory.SelectedItem;
            var ID = ib.ID;
        ...
        private void btnDeleteCancel_Click(object sender, EventArgs e)
        {
            ...
            {
                string failureBox = "Error: product does not exist in the database.";
                MessageBox.Show(failureBox);
                this.Close();
            }
        }
        ...
        {
            string messageDelete = "Do you want to delete this product?";
            string title = "Close Window";
            MessageBoxButtons closeButtons = MessageBoxButtons.YesNo;
            DialogResult result = MessageBox.Show(messageDelete, title, closeButtons);
